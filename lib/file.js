...
client.init = function() {
  ...
  if (!client[config.queue.uploadsLabel]) {
    client.uploads = new QUEUE(config.queue.uploadsLabel, config.queue.storageType) || {};
    deferredInitQueues.resolve();
  } else if (client[config.queue.uploadsLabel].ready) {
    deferredInitQueues.resolve();
  }

  client.initPromise = q.all([deferredFhinit.promise, deferredReady.promise, deferredInitQueues.promise]);
...
client.processQueue = function() {
  return client.initPromise.then(function() {
    return client[config.queue.uploadsLabel].recoverQueue().then(function(processingQueue) {
      if (processingQueue) {
        client[config.queue.uploadsLabel].queue.forEach(function(item) {
