...
client.init = function() {
  ...
  // var deferredFSReady = q.defer();

  // fsModule.init().then(function(res) {
  //   client.fs = res.fs;
  //   fsModule.ready = res.ready;
  //   deferredFSReady.resolve();
  // }, function() {
  //   client.fs = null;
  //   deferredFSReady.reject();
  // });
...
client.getFs = function() {
  var d = q.defer();
  d.resolve(fsModule.fs);
  return d.promise;
};
    ...
    if (processingQueue) {
      client.uploads.queue.forEach(function(item) {
        if (window && window.cordova) {
          ...
          client.uploadFile(item.userId, item.filePath, {
            fileName: item.fileName
          }).then(function() {
            return client.uploads.removeItem(item);
          }, function() {
            return client.uploads.updateItem(item.id, 'status', 'retry');
          });
        } else {

          client.uploadDataUrl(item.userId, item.filePath).then(function() {
            return client.uploads.removeItem(item);
          }, function() {
            return client.uploads.updateItem(item.id, 'status', 'retry');
          });
